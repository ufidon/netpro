...
def main(args):
    ...
    if not args.i:
        ...
        # If you call add_alternative() at least once, then 
        # a multipart/alternative container is created to hold the original 
        # body together with the alternative part(s) you add
        message.add_alternative(plain)
    else:
        cid = email.utils.make_msgid()  # RFC 2392: must be globally unique!
        #  Base64 encoding has been used for binary data-like images 
        # that are not 7-bit safe
        message.set_content(html + img.format(cid.strip('<>')), subtype='html')

        # If you call add_related() at least once, then the body you specified 
        # with set_content() will get grouped together with all of the related 
        # content inside a single multipart/related section
    ...
    for filename in args.filename:
        ...
        if main == 'text':
            with open(filename, encoding='utf-8') as f:
                text = f.read()
            # if you call add_attachment() at least once, then an outer 
            # multipart/mixed container is generated to hold the content 
            # next to all of the attachments you add
